...
// Yhteyden luominen
$sql = "INSERT INTO ari1606_kirjat (nimi, kategoria, hinta, isbn, kunto) 
VALUES ('" . $nimi . "', '" . $kategoria . "', '" . $uusi_hinta . "', '" . $isbn . "', '" . $kunto . "')";
// Ajetaan komento tietokantapalvelimella
if(mysqli_query($yhteys, $sql)) {
	$sql = "SELECT * FROM ari1606_kirjat WHERE ID = (SELECT MAX(ID) FROM ari1606_kirjat)";
	if(!$vastaus = mysqli_query($yhteys, $sql)) {
	die("Virhe SQL-kyselyssä!" . mysqli_error($yhteys));
}
	while($rivi = mysqli_fetch_assoc($vastaus)) {
	echo "<h1 style='padding-left: 30px'><br/><br/>Kirjan lisääminen onnistui!</h1>";
	echo "<hr><br/>";
